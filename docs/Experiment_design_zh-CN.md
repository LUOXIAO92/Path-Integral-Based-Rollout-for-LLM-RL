...
MCMC 接受率:
```math
A_k(\tau\rightarrow\tau') = \min\left(1,\exp[-(S_\eta[\tau']-S_\eta[\tau])]\frac{q_{\mathrm{prop}}(\tau\mid\tau')}{q_{\mathrm{prop}}(\tau'\mid\tau)}\right)
```

若暂时不计算提案比值，则使用 $\rho_{\mathrm{prop},k}\in(0,1]$ 近似:
```math
A_k(\tau\rightarrow\tau') = \min\left(1,\rho_{\mathrm{prop},k}\exp[-(S_\eta[\tau']-S_\eta[\tau])]\right)
```
...
路径级回报由第 7 节的奖励评价与计算规则得到。若只裁判一次，则 $G[\tau]=r_0$ 。若同一条路径裁判 $M_G$ 次，则:
```math
...
因此 $0\leq N[\tau]<1$ 。当 $L_\tau\le L_{max}$ 时，$N[\tau]=0$ 。

#### 1.1.4 基础路径测度 $S_0[\tau]$

令 $\pi_{\theta_t}$ 表示当前策略模型。对路径 $\tau=[a_0,o_1,r_0]$ ，基础路径测度取完整动作 $a_0$ 的负对数概率:
```math
S_0[\tau]=-\sum_{i=L_0}^{L_1-1}\log \pi_{\theta_t}(a_i\mid h_{i,0})
```

其中 $h_{i,0}=[h_0,a_{L_0},\ldots,a_{i-1}]$ 。这里的 $\pi_{\theta_t}$ 使用 raw logprob，也就是原始 logits 在完整词表上的 softmax 分布:
```math
\log \pi_{\theta_t}(a_i\mid h_{i,0}) = z_{i,a_i}-\log\sum_{v\in V}\exp(z_{i,v})
```
...
```math
K[\tau]=\sum_{i=L_0}^{L_1-1}\log\frac{\pi_{\theta_t}(a_i\mid h_{i,0})}{\pi_{\mathrm{ref}}(a_i\mid h_{i,0})}
```
...
代入各项定义:
```math
F[\tau]=\lambda_GG[\tau]-\lambda_NN[\tau]-\lambda_{KL}K[\tau]
```
...
令 $\Delta S_\eta=S_\eta[\tau']-S_\eta[\tau]$ ，近似接受率为:
```math
A_k(\tau\rightarrow\tau')=\min\left(1,\rho_{\mathrm{prop},k}e^{-\Delta S_\eta}\right)
```

采样 $u_k\sim \mathrm{Uniform}(0,1)$ 。若 $u_k\le A_k(\tau\rightarrow\tau')$ ，则接受候选路径 $\tau'$；否则拒绝，链状态保持为原路径 $\tau$ 。

#### 1.1.8 best-of-n 的两个选择目标

对同一问题前缀 $h_0=[o_0]$ 采样得到 $\tau_1,\ldots,\tau_n$ 。按最低作用量选择:
```math
\tau^{(S)} = \arg\min_{\tau_b,\ b=1,\ldots,n} S_\eta[\tau_b]
```

按最高回报选择:
```math
\tau^{(G)} = \arg\max_{\tau_b,\ b=1,\ldots,n} G[\tau_b]
```

$\tau^{(S)}$ 用于比较作用量目标，$\tau^{(G)}$ 用于比较回报目标，二者分开报告。
...
单步长链推理取 $T=0$ ，问题作为初始历史 $h_0=[o_0]$ ，完整回答作为一个阶段级动作 $a_0=[a_{L_0},\ldots,a_{L_1-1}]$ ，路径为 $\tau=[a_0,o_1,r_0]$ 。第一阶段只用问题作为候选动作前缀 $c'_a=h_0$ ，不扰动观测，也就是 $o'_{\mathrm{allow}}=o_{\mathrm{allow}}$ 。候选路径
```math
\begin{aligned}
...
每次生成候选路径 $\tau'$ ，都按第 7 节计算 $G[\tau']$，并计算 $N[\tau'], K[\tau'], F[\tau'], S_0[\tau'], S_\eta[\tau']$ ，再用 $A_k(\tau\rightarrow\tau')$ 决定接受或拒绝。实验中必须保存
* 全部候选路径
* accepted 候选路径
* rejected 候选路径
* MCMC 链状态路径
...
作用量筛选效果 $\mathrm{mean}_{all}S_\eta[\tau] - \mathrm{mean}_{accepted}S_\eta[\tau]$ ，其中 accepted / all / rejected 的平均作用量如下
```math
\begin{aligned}
& \mathrm{mean}_{accepted}S_\eta[\tau] \\
& \mathrm{mean}_{all}S_\eta[\tau] \\
& \mathrm{mean}_{rejected}S_\eta[\tau]
\end{aligned}
```
...
分解项:
```math
G[\tau],\quad N[\tau],\quad S_0[\tau],\quad S_\eta[\tau]
```
...
4. 作用量下降是否伴随 $G[\tau]$ 提升，而不是只来自 $N[\tau]$ 下降
...
第二阶段需要验证的是: *不训练模型，只通过续写某些步骤级 prefix，是否能得到比 best-of-n 更稳定的回答*。这里的“稳定”指的是多次续写时的正确率和回报是否能稳定下来，并且保持高正确率。
...
```math
\begin{aligned}
& \mathrm{mean}\ S_\eta[\tau'] \\
& \mathrm{Var}\ S_\eta[\tau'] \\
& \mathrm{mean}\ G[\tau'] \\
& \mathrm{mean}\ N[\tau']
\end{aligned}
```
...
其中 `prefix_continuation_error_rate` ，即续写错误率。回答错误、推理错误、最终答案错误、回答偏离问题，都计为错误。
...
第一阶段: $h_0 \rightarrow \mathrm{Rollout} \rightarrow \tau' \rightarrow A_k(\tau\rightarrow\tau')$

* 目标是验证问题前缀下的路径级 MCMC 是否有效。

第二阶段: $\tau \rightarrow c'_a \rightarrow \mathrm{Rollout} \rightarrow \tau' \rightarrow A_k(\tau\rightarrow\tau')$
...
4. 第二阶段 best-of-n 路径与 MCMC 接受路径的前缀续写正确率
5. 第二阶段 best-of-n 路径与 MCMC 接受路径的前缀续写错误率
6. 第二阶段前缀续写后的 $S_\eta[\tau']$ 均值与方差
7. 对孤立尖峰与稳定路径盆地的区分
...
第一阶段主采样 backend 使用 vLLM。vLLM 负责批量 rollout、高温采样、返回生成 token 和 raw logprob。当前流程是:
```text
vLLM 生成路径
→ DeepSeek-V4-Pro 裁判
...
10. 计算 A_k(τ→τ')
11. 保存 accepted / rejected / chain endpoint
12. 用同样 rollout 预算生成 best-of-n
13. 生成第一阶段统计报告
```
...
`problem_analysis` 用于分析题目要求。`task_target` 写题目真正要解决什么，`required_output` 写最终答案应该输出什么，`hard_constraints` 写题目里的硬约束，`possible_off_task_patterns` 只写少量可预见的偏题情况，不要求穷举。
...
推理过程分为:
```math
R_{\mathrm{process}}[\tau]=\frac{\sum_j w_js_j}{\sum_j w_j}
```

若 $\sum_j w_j=0$，则:
```math
R_{\mathrm{process}}[\tau]=\mathrm{no\_scored\_span\_process\_score}
```

当前默认权重会降低无关步骤和非关键步骤的取巧收益。
