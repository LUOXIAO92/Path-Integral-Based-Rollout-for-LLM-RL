...
from src.mcmc import StrictMCMCActionConfig, run_mcmc_chain, select_best_of_n
...
def main() -> None:
    ...
    strict_action_config = load_strict_action_config()
...
def load_strict_action_config() -> StrictMCMCActionConfig | None:
    if PROPOSAL_RATIO_MODE != "strict":
        return None
    with SCORING_CONFIG_JSON.open("r", encoding="utf-8") as handle:
        scoring_config = ScoringConfig.model_validate(json.load(handle))
    return StrictMCMCActionConfig(
        eta=scoring_config.eta,
        lambda_g=scoring_config.lambda_G,
        lambda_n=scoring_config.lambda_N,
        lambda_kl=scoring_config.lambda_KL,
